...
function Home(props) {
    ...
    const [catagoriesbackground, setbackgroundcatagories] = useState('');
    const [showreview, setreview] = useState('Sally Ride');

    useEffect(() => {
        const endpoint = process.env.REACT_APP_ANALYTICS_ENGINE + '/query/product?limit=5';
      ...
      const popularproducts_indicator = (type) => {
            console.log(type);
            setbackground(type);
      };

      const catagory_indicator = (event) => {

        if ( event.currentTarget.textContent == 'Shampoo') {
            setbackgroundcatagories([{'Shampoo': 'red', 'Conditioner':"", 'Face_Care': ""}])
        } else if (event.currentTarget.textContent == 'Conditioner') {
            setbackgroundcatagories([{'Shampoo': '', 'Conditioner':"red", 'Face_Care': ""}])
        } else {
            setbackgroundcatagories([{'Shampoo': '', 'Conditioner':"", 'Face_Care': "red"}])
        }

        //let currentCategories = catagoriesbackground;
        //let newCategories = [...currentCategories, {'Shampoo': '', 'Conditioner':"", 'Face_Care': "red"}]
        
        console.log(catagoriesbackground[0]['Shampoo'])
    };

      const reviewRender = (type) => {
          setreview(type)
      }
            ...
            <section className="popularproducts">
                ...
                <div className="catagories">
                    {backgrounds === 'top_selling' ? (
                        <div className="textbackground"  onClick={() => popularproducts_indicator('top_selling')}>
                    ...
                    {backgrounds === 'top_rated' ? (
                        <div className="textbackground"  onClick={() => popularproducts_indicator('top_rated')}>
                            <img  className="splashImg" src={splash} />
                    ...
                    {backgrounds === 'recommendation' ? (
                        <div className="textbackground"  onClick={() => popularproducts_indicator('recommendation')}>
                            <img  className="splashImg" src={splash} />
