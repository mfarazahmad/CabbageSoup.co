...
function ViewModifyInventory(props) { 
  ...
  useEffect(() => {
      axios.get(process.env.REACT_APP_ANALYTICS_ENGINE + "/query/product")
      .then((response) => {
  ...
  const handleOnFileLoad = (data) => {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      let URL = process.env.REACT_APP_ANALYTICS_ENGINE + "/query/product";
    ...
    const handleDelete = (record) => {
      let url = process.env.REACT_APP_ANALYTICS_ENGINE + `/query/product/${record.product_id}`
      let headers = {
          "Access-Control-Allow-Origin": "*",
